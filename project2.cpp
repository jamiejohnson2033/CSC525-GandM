...

{
	...
	/*JJ ADDDED THE HELP OPTION */
	case 5:
		glutSetWindow(2); // changing the current window to the "Info Window"
		glutShowWindow(); // displaying the Help Window
		break;
...
void infoInstructions()
{
	...
	string myArray[6] = {
		"DISPLAY TEXT:  Place the cursor within the window and type the text of your choice. ",
		"CHANGE COLOR/FONT STYLE:  Right click the mouse button in the main window and make your selection. ",
		"MOVE MESSAGE:  While pressing the left mouse button (within the window), move around the cursor. ",
		"CLOSE MAIN WINDOW:  Select the Close option from the drop down list.",
		"OPEN/RE-OPEN HELP WINDOW:  Right click the mouse button and select the Help option.",
		"CLOSE HELP WINDOW:  Right click the mouse button within the Help window and select the Close option.",
	};

	for (int i = 0; i < 6; i++)
	{
		glRasterPos2i(-375, 150 - 50 * i); // position 
...
int main(int argc, char **argv){
	...
	/*JJ ADDDED THE "HELP" OPTION BELOW*/
	...
	glutInitWindowSize(640, 200);			// specify a window size
	glutInitWindowPosition(500, 0);			// specify a window position
	int window2 = glutCreateWindow("Info");		// create a titled window
	//cout << "window2 number: " << window2;
	Init();									// setting up
